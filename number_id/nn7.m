...
ResA = zeros(7,7,size(A,3));
ResTestA = zeros(7,7,size(TestA,3));
%reshape A
for i = 1:size(A,3)
    ResA(:,:,i) = imresize ( A(:,:,i), 0.25 );
end

for i=1:size(TestA,3)
    ResTestA(:,:,i) = imresize( TestA(:,:,i), 0.25 );
end
...
save ResultsDown_NN.mat Results