...
load DigitRP
load ResultsRP_NN.mat
%Results = zeros(4,size(DigitRP,2));

%for runs=1:size(DigitRP,2)
for runs=3
    % seperate training and dataset
    DigitRP{2,runs}
    ptrans = DigitRP {3,runs};
    A = ptrans(:, 1:numA);
    TestA = ptrans(:, numA+1:end);
